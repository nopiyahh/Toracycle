	...
	function getQuote($address) {
		...
		if ($status) {
			...
			$quote_data['fedex'] = array(
				...
				'cost'         => $this->config->get('shipping_fedex_cost'),
				'tax_class_id' => $this->config->get('shipping_fedex_tax_class_id'),
				'text'         => $this->currency->format($this->tax->calculate($this->config->get('shipping_fedex_cost'), $this->config->get('shipping_fedex_tax_class_id'), $this->config->get('config_tax')), $this->session->data['currency'])
			);

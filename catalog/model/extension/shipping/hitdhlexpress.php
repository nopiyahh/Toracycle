...
// echo '<pre>'; print_r($weight_query);die();
			...
			$pay_con = $this->config->get('hitdhlexpress_country_code');

			if (!empty($this->config->get('hitdhlexpress_pay_con_type')) && $this->config->get('hitdhlexpress_pay_con_type') == "R") {
				$pay_con = $address['iso_code_2'];
			}elseif (!empty($this->config->get('hitdhlexpress_pay_con_type')) && $this->config->get('hitdhlexpress_pay_con_type') == "C") {
				if (!empty($this->config->get('hitdhlexpress_cus_pay_con_type'))) {
					$pay_con = $this->config->get('hitdhlexpress_cus_pay_con_type');
				}
			}
			...
			$xml .= '		</From>';
			$xml .= '		<BkgDetails> ';
			$xml .= '			<PaymentCountryCode>'.$pay_con.'</PaymentCountryCode>';

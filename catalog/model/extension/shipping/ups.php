	...
	function getQuote($address) {
		...
		if ($status) {
			...
			$quote_data['ups'] = array(
				...
				'cost'         => $this->config->get('shipping_ups_cost'),
				'tax_class_id' => $this->config->get('shipping_ups_tax_class_id'),
				'text'         => $this->currency->format($this->tax->calculate($this->config->get('shipping_ups_cost'), $this->config->get('shipping_ups_tax_class_id'), $this->config->get('config_tax')), $this->session->data['currency'])
			);
